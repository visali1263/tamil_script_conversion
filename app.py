...
def upload():
    ...
    if request.method == 'POST':
        imagefile = request.files['image']  # uploaded image file
        filename = werkzeug.utils.secure_filename(imagefile.filename)
        print(filename)
        imagefile.save("C:/" + filename)
        return jsonify({
            "message": "image uploaded successfully, Please wait..."
        })
    

def ai_code(filename):
    tf.config.experimental.list_physical_devices('GPU')
    tf.test.is_gpu_available()

    tf.config.list_physical_devices('GPU')
    device_lib.list_local_devices()
    string = r"F:/fileUpload/upload" + filename
    print(string)
    image = cv2.imread(string) # input image location
...
def donwload():
    if request.method == 'GET':
        img = cv2.imread("public/Output/final.png")  # output image file
        jpg_img = cv2.imencode('.jpg', img)
        b64_string = base64.b64encode(jpg_img[1]).decode('utf-8')
        return jsonify({"opimage": b64_string})
